<template>
  <base-page :title="title">
    <f7-block strong>
      <p>
        更多用法参考：<f7-link
          href="https://framework7.io/vue/stepper.html"
          text="https://framework7.io/vue/stepper.html"
          external
        ></f7-link>
      </p>
    </f7-block>
    ...
    <f7-block-header
      >Pressing and holding one of its buttons increments or decrements the
      stepper’s value repeatedly. With dynamic autorepeat, the rate of change
      depends on how long the user continues pressing the
    ...
    <f7-block-header
      >In wraps mode incrementing beyond maximum value sets value to minimum
      value, likewise, decrementing below minimum value sets value to maximum
      value</f7-block-header
    >
    <f7-block strong class="text-align-center">
      <f7-row>
        <f7-col>
    ...
    <f7-block-header
      >It is possible to enter value manually from keyboard or mobile keypad.
      When click on input field, stepper enter into manual input mode, which
      allow type value from keyboard and check fractional part with defined
      accurancy. Click outside or enter Return key, ending manual
...
export default defineComponent({
  ...
  setup() {
    ...
    const meetingTimeComputed = computed(() => {
      const value = state.meetingTime;
    ...
    const setApples = (value: number) => (state.applesCount = value);
    const setOranges = (value: number) => (state.orangesCount = value);
    const setMeetingTime = (value: number) => (state.meetingTime = value);
