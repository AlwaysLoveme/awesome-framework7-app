<template>
  <base-page :title="title">
    <f7-block strong>
      更多用法参考：<f7-link
        href="https://framework7.io/vue/progressbar.html"
        text="https://framework7.io/vue/progressbar.html"
        external
      ></f7-link>
    </f7-block>
    ...
    <f7-block strong>
      <p>
        When progress bar is determinate it indicates how long an operation will
        take when the percentage complete is detectable.
      </p>
      ...
      <div>
        ...
        <p>
          <f7-button raised @click="showDeterminate(true)">
            Start Loading
          </f7-button>
        </p>
      </div>
      <div>
        ...
        <p>
          <f7-button raised @click="showDeterminate(false)"
            >Start Loading</f7-button
          >
        </p>
      </div>
    </f7-block>
    ...
    <f7-block strong>
      <p>
        When progress bar is infinite/indeterminate it requests that the user
        wait while something finishes when it’s not necessary to indicate how
        long it will take.
      </p>
      ...
      <div>
        ...
        <p>
          <f7-button raised @click="showInfinite(false)"
            >Start Loading</f7-button
          >
        </p>
      </div>
      <div>
        ...
        <p>
          <f7-button raised @click="showInfinite(true)"
            >Start Loading</f7-button
          >
        </p>
      </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const setInlineProgress = function (value: number) {
      f7.progressbar.set("#demo-inline-progressbar" as any, value);
    };
    const showDeterminate = function (inline: any) {
      if (state.determinateLoading) return;
      state.determinateLoading = true;
      let progressBarEl: any;
      if (inline) {
        progressBarEl = f7.progressbar.show("#demo-determinate-container", 0);
      } else {
        progressBarEl = f7.progressbar.show(
          0,
          f7.theme === "md" ? "yellow" : "blue"
        );
      }
      let progress = 0;
    ...
    const showInfinite = function (multiColor: boolean) {
